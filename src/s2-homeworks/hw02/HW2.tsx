...
// constants
const defaultAffairs: AffairType[] = [ // need to fix any  OK
...
export const filterAffairs = (affairs: AffairType[], filter: FilterType): any => { // need to fix any
    let filteredAffairs = affairs.filter(el => {
        if (filter === 'all') {
            return affairs
        } else {
...
export const deleteAffair = (affairs: AffairType[], _id: number): AffairType[] => { // need to fix any  ЧТО ТО С ТИПОМ
    let newAffairs = affairs.filter(el => _id !== el._id)
    console.log(_id)
...
function HW2() {
    const [affairs, setAffairs] = useState<AffairType[]>(defaultAffairs) // need to fix any  OK
    const [filter, setFilter] = useState<FilterType>('all')

    const filteredAffairs = filterAffairs(affairs, filter)
    const deleteAffairCallback = (_id: number) => { // need to fix any
        // debugger
        // need to fix ОПИСАТЬ TYPES
